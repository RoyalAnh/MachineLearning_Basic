...
y = np.random.randint(0, 3, (N,)) # # Tạo mảng y chứa các nhãn ngẫu nhiên từ 0 đến 3
...
def grad(X, Y, W): # Tính gradient theo lý thuyết
    A = softmax((W.T.dot(X)))
    E = A - Y
    return X.dot(E.T)
    
def numerical_grad(X, Y, W, cost): # Tính gradient theo phương pháp số
    eps = 1e-6
    g = np.zeros_like(W)
...
# Hàm thực hiện huấn luyện hồi quy Softmax
